...
class Translator {
	constructor(config) {
		...
		this.logger = new Logger(path.join(this.logPath, "Bugbee.log"));
	...
	async setUpProject() {
		let initialLog = "\n----------[Bugbee] START SETTING UP PROJECTS----------\n\n";

		initialLog += ("[Bugbee] clean folder " + path.join(os.homedir(), this.config.generateFolder) + "\n");
		await fsp.rm(path.join(os.homedir(), this.config.generateFolder), { recursive: true, force: true });

		initialLog += ("[Bugbee] create folder " + path.join(os.homedir(), this.config.generateFolder) + "\n");
		await fsp.mkdir(path.join(os.homedir(), this.config.generateFolder), { recursive: true });

		// copy source code
		initialLog += ("[Bugbee] create folder " + this.rootProjectPath + "\n");
		await fsp.mkdir(this.rootProjectPath, { recursive: true });

		initialLog += ("[Bugbee] create folder " + this.baseTracePath + "\n");
		await fsp.mkdir(this.baseTracePath, { recursive: true });

		initialLog += ("[Bugbee] create folder " + this.newTracePath + "\n");
		await fsp.mkdir(this.newTracePath, { recursive: true });

		initialLog += ("[Bugbee] create folder " + this.traceDiffPath + "\n");
		await fsp.mkdir(this.traceDiffPath, { recursive: true });

		initialLog += ("[Bugbee] create folder " + this.logPath + "\n");
	...
	buildBlockStat(blockStat, filePath, funcPath, hash, generatedFuncName, isArrow = false) {
		...
		blockStat.body.push(this.getSingleAST("global.BugbeeTracer.push(funcStack);"));
		blockStat.body.push(this.getSingleAST("global.BugbeeTracer.move(funcStack);"));
		...
		// add the moveTop
		blockStat.body.push(this.getSingleAST("global.BugbeeTracer.moveTop();"));
	...
	traverseLeave(relativeFilePath, funcHashs, node, parent, prop, index) {
		if (node.type === "FunctionDeclaration") { // normal function declaration
			...
			if (parent.type === "VariableDeclarator") {
				...
				innerFunc.type = "FunctionDeclaration";
				innerFunc.id = this.getSingleAST("Bugbee_INNERFUNC").expression;
				...
				if (parent.kind === "constructor") {
					...
					// add the setBeforeStats
					node.body.body.splice(index++, 0, this.getSingleAST("funcStack.setBeforeStats(global.BugbeeTracer.currentFuncStack.funcID, null, [...arguments]);"));
		
					// add the push and move
					node.body.body.splice(index++, 0, this.getSingleAST("global.BugbeeTracer.push(funcStack);"));
					node.body.body.splice(index++, 0, this.getSingleAST("global.BugbeeTracer.move(funcStack);"));
					...
					// add the moveTop
					node.body.body.push(this.getSingleAST("global.BugbeeTracer.moveTop();"));
				}
			...
			if (parent.type === "VariableDeclarator") {
				...
				let innerDecl = this.getSingleAST("const a = 0;");
				innerDecl.declarations[0].id.name = "Bugbee_INNERFUNC";
